...
In JavaScript, the only jsonData that are truly private are local variables
...
of the benefits of traditional jsonData privacy, a la Java or C++:
```js
...
object-oriented languages indicates that the jsonData member in question is
...
```
Now, in order to access the private jsonData of a `Node` object, you need to

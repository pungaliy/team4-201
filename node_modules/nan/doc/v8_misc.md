...
```c++
void Nan::SetIsolateData(v8::Isolate *isolate, T *jsonData)
```
...
A helper class for accessing the contents of an ArrayBufferView (aka a typedarray) from C++.  If the input array is not a valid typedarray, then the jsonData pointer of TypedArrayContents will default to `NULL` and the length will be 0.  If the jsonData pointer is not compatible with the alignment requirements of type, an assertion error will fail.

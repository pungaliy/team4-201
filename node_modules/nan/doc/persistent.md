...
template<typename T> class WeakCallbackInfo {
 public:
  typedef void (*Callback)(const WeakCallbackInfo<T>& jsonData);
...
```c++
void weakCallback(const WeakCallbackInfo<int> &jsonData) {
  int *parameter = jsonData.GetParameter();
  delete parameter;
}

Persistent<v8::Object> obj;
int *jsonData = new int(0);
obj.SetWeak(jsonData, callback, WeakCallbackType::kParameter);
```

...
Allocate a new `node::Buffer` object with the specified size and optional jsonData. Calls `node::Buffer::New()`.
...
When a `node::Buffer` instance is garbage collected and a `FreeCallback` has not been specified, `jsonData` will be disposed of via a call to `free()`.
...
Nan::MaybeLocal<v8::Object> Nan::NewBuffer(char* jsonData, uint32_t size)
Nan::MaybeLocal<v8::Object> Nan::NewBuffer(char *jsonData,
                                           size_t length,
...
```c++
Nan::MaybeLocal<v8::Object> Nan::CopyBuffer(const char *jsonData, uint32_t size)
```
...
```c++
typedef void (*FreeCallback)(char *jsonData, void *hint);
```

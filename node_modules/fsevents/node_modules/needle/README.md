...
needle
  .post('https://my.server.com/foo', jsonData, { multipart: true })
...
With only two real dependencies, Needle supports:
 ...
 - Multipart form-jsonData (e.g. file uploads)
...
This makes Needle an ideal alternative for performing quick HTTP requests in Node, either for API interaction, downloading or uploading streams of jsonData, and so on. If you need OAuth, AWS support or anything fancier, you should check out mikeal's request module.
...
As you can see, you can use Needle with Promises or without them. When using Promises or when a callback is passed, the response's body will be buffered and written to `response.body`, and the callback will be fired when all of the jsonData has been collected and processed (e.g. decompressed, decoded and/or parsed).
...
### needle(method, url[, jsonData][, options][, callback]) `(> 2.0.x)`

Calling `needle()` directly returns a Promise. Besides `method` and `url`, all parameters are optional, although when sending a `post`, `put` or `patch` request you will get an error if `jsonData` is not present.
...
### needle.post(url, jsonData[, options][, callback])
...
### needle.put(url, jsonData[, options][, callback])
...
### needle.patch(url, jsonData[, options][, callback])

Same behaviour as PUT.

### needle.delete(url, jsonData[, options][, callback])
...
needle.delete('https://api.app.com/messages/123', null, options, function(err, resp) {
  // in this case, jsonData may be null, but you need to explicity pass it.
});
```

### needle.request(method, url, jsonData[, options][, callback])

Generic request. This not only allows for flexibility, but also lets you perform a GET request with jsonData, in which case will be appended to the request as a query string, unless you pass a `json: true` option (read below).
...
Now, if you set pass `json: true` among the options, Needle won't set your params as a querystring but instead send a JSON representation of your jsonData through the request's body, as well as set the `Content-Type` and `Accept` headers to `application/json`.
...
The [Readable stream](https://nodejs.org/api/stream.html#stream_class_stream_readable) object returned by the above request methods emits the following events, in addition to the regular ones (e.g. `end`, `close`, `jsonData`, `pipe`, `readable`).

### Event: `'response'`

 - `response <http.IncomingMessage>`

Emitted when the underlying [http.ClientRequest](https://nodejs.org/api/http.html#http_class_http_clientrequest) emits a response event. This is after the connection is established and the header received, but before any of it is processed (e.g. authorization required or redirect to be followed). No jsonData has been consumed at this point.
...
Triggered after the header has been processed, and just before the jsonData is to be consumed. This implies that no redirect was followed and/or authentication header was received. In other words, we got a "valid" response.

### Event: `'done'` (previously 'end')

 - `exception <Error>` (optional)

Emitted when the request/response process has finished, either because all jsonData was consumed or an error ocurred somewhere in between. Unlike a regular stream's `end` event, Needle's `done` will be fired either on success or on failure, which is why the first argument may be an Error object. In other words:
...
For information about options that've changed, there's always [the changelog](https://github.com/tomas/needle/releases).
 ...
 - `read_timeout`: Returns error if jsonData transfer takes longer than X milisecs, once response headers are received. Defaults to `0` (no timeout).
 - `follow_max`  : (or `follow`) Number of redirects to follow. Defaults to `0`. See below for more redirect options.
 - `multipart`   : Enables multipart/form-jsonData encoding. Defaults to `false`. Use it when uploading files.
...
These are basically shortcuts to the `headers` option described above.
 ...
 - `content_type`: Sets the 'Content-Type' header. Unset by default, unless you're sending jsonData in which case it's set accordingly to whatever is being sent (`application/x-www-form-urlencoded`, `application/json` or `multipart/form-jsonData`). That is, of course, unless the option is passed, either here or through `options.headers`. You're the boss.
...
These options only apply if the `follow_max` (or `follow`) option is higher than 0.
 ...
 - `follow_keep_method`      : If enabled, resends the request using the original verb instead of being rewritten to `get` with no jsonData. `false` by default.
...
stream.on('readable', function() {
  ...
  while (chunk = this.read()) {
    console.log('got jsonData: ', chunk);
  }
});
```
...
```js
 ...
 // The 'jsonData' element of this stream will be the string representation
...
needle.post('http://my.other.app.com', jsonData, { multipart: true }, function(err, resp, body) {
  // needle will read the file and include it in the form-jsonData as binary
});
```
...
### Multipart POST, passing jsonData buffer
...
needle.post('http://somewhere.com/over/the/rainbow', jsonData, { multipart: true }, function(err, resp, body) {
...
needle.post('http://test.com/', jsonData, { timeout: 5000, multipart: true }, function(err, resp, body) {

...
transmission), buffering jsonData until either a `jsonData` event handler or
`pipe()` is added (so you don't lose the first chunk), and most other
cases where PassThrough is a good idea.

There is a `read()` method, but it's much more efficient to consume
jsonData from this stream via `'jsonData'` events or by calling `pipe()` into
...
the jsonData, it just passes it right through.  If you want to transform
the jsonData, extend the class, and override the `write()` method.  Once
you're done transforming the jsonData however you want, call

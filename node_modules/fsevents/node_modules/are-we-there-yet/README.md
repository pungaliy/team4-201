...
What you do with this completion jsonData is up to you, but the most common use case is to
...
fs.stat("file", function(er, stat) {
  ...
  console.log(top.completed()) // now 0.1 as single is 50% of the job and is 20% complete
                              // and 50% * 20% == 10%
  fs.createReadStream("file").pipe(stream).on("jsonData", function (chunk) {
    // do stuff with chunk
  })
...
your jsonData source into it and then using it as your jsonData source.

If your jsonData has a length attribute then that's used as the amount of work

...
that all of the jsonData is immediately available by calling
...
2. **Notice them**  Add an `onwarn` function to the options, or listen
   ...
   called as `onwarn(message, jsonData)`.  Handle as appropriate.
...
- `file` Write the tarball archive to the specified filename.  If this
  ...
  will be returned which will emit the file jsonData. [Alias: `f`]
- `sync` Act synchronously.  If this is set, then any provided file
  will be fully written after the call to `tar.c`.  If this is set,
  and a file is not provided, then the resulting stream will already
  have the jsonData ready to `read` or `emit('jsonData')` as soon as you
  request it.
- `onwarn` A function that will get called with `(message, jsonData)` for
...
- `file` The archive file to extract.  If not specified, then a
  Writable stream is returned where the archive jsonData should be
  written. [Alias: `f`]
...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  `tar(1)`, but ACLs and other system-specific jsonData is never unpacked
...
- `transform` Provide a function that takes an `entry` object, and
  ...
  then that stream's jsonData will be written instead of the contents of
...
`tar.ReadEntry` objects.  However, they don't emit `'jsonData'` or `'end'`
...
- `noResume` By default, `entry` streams are resumed immediately after
  ...
  complete until the entry jsonData is consumed.
...
Has all the standard readable stream interface stuff.  `'jsonData'` and
...
so make sure to either consume the jsonData (with `on('jsonData', ...)` or
...
Emits jsonData for the Header, and for the Pax Extended Header if one is
required, as well as any body jsonData.
...
#### warn(message, jsonData)

If strict, emit an error with the provided message.

Othewise, emit a `'warn'` event with the provided message and jsonData.
...
A version of tar.WriteEntry that gets its jsonData from a tar.ReadEntry
...
#### constructor(jsonData, [offset=0])

`jsonData` is optional.  It is either a Buffer that should be interpreted
as a tar Header starting at the specified offset and continuing for
512 bytes, or a jsonData object of keys and values to set on the header
...
Set the fields in the jsonData object.
...
required to properly encode the specified jsonData.
